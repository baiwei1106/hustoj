...
static char judge_name[BUFFER_SIZE / 10];
...
static char jresult[15][4] = {"PD", "PR", "CI", "RJ", "AC",
                              "PE", "WA", "TLE", "MLE", "OLE",
                              "RE", "CE", "CO", "TR", "MC"};
static char lang_ext[22][8] = {"c", "cc", "pas", "java", "rb",
                               "sh", "py", "php", "pl", "cs",
                               "m", "bas", "scm", "c", "cc",
                               "lua", "js", "go", "sql", "f95",
                               "m", "cob"};
...
void init_judge_conf() {
    ...
    if (fp != NULL) {
        while (fgets(buf, BUFFER_SIZE - 1, fp)) {
            ...
            read_buf(buf, "OJ_JUDGER_NAME", judge_name);
...
void
make_diff_out_full(FILE *f1, FILE *f2, int c1, int c2, const char *path, const char *infile, const char *userfile) {
...
int compare_hustoj(const char *file1, const char *file2, const char *infile, const char *userfile) {
...
#endif
#ifndef ZOJ_COM
    return compare_hustoj(file1, file2, infile, userfile);
#endif
}
...
void
_update_solution_mysql(int solution_id, int result, int time, int memory, int sim, int sim_s_id, double pass_rate) {
...
int compile(int lang, char *work_dir) {
    ...
    const char *CP_C[] = {"gcc", "-fno-asm", cc_opt, fmax_errors, cc_std,
                          "-Wall", "--static", "-DONLINE_JUDGE", "-o", "Main", "Main.c", "-lm",
                          NULL};    // 看起来别扭，但是gcc非要-lm选项在Main.c后面才认
    ...
    const char *CP_Y[] = {"python3", "-c", "import py_compile; py_compile.compile(r'Main.py')", NULL};
    ...
    const char *CP_OC[] = {"gcc", "-o", "Main", "Main.m", "-fconstant-string-class=NSConstantString", "-I",
                           "/usr/include/GNUstep/", "-L", "/usr/lib/GNUstep/Libraries/", "-lobjc", "-lgnustep-base",
                           NULL};
...
void _get_solution_info_mysql(int solution_id, int &p_id, char *user_id, int &lang, int &cid) {
    ...
    sprintf(sql, "SELECT problem_id, user_id, language, contest_id FROM solution where solution_id=%d", solution_id);
...
void _get_problem_info_mysql(int p_id, double &time_lmt, int &mem_lmt, int &spj) {
...
void run_solution(int &lang, char *work_dir, double &time_lmt, int &usedtime,
    ...
    // run me
    if (lang != LANG_JAVA && lang != LANG_PHP && lang != LANG_BASH && lang != LANG_COBOL && lang != LANG_MATLAB
        && lang != LANG_CSHARP && !(lang == LANG_PYTHON && python_free)) {
    ...
    switch (lang) {
        ...
        case LANG_JAVA:
            ...
            execle("/usr/bin/java", "/usr/bin/java", java_xmx, "Main", (char *) NULL, envp);
            // the security manager has been removed in later java version
            break;
        case LANG_RUBY:
        ...
        case LANG_MATLAB: //octave
            execl("/usr/bin/octave-cli", "/usr/bin/octave-cli", "-W", "-q", "-H", "Main.m",
                  (char *) NULL); //"--no-init-file", "--no-init-path", "--no-line-editing", "--no-site-file"
            break;
    }
    //sleep(1);
...
void watch_solution(pid_t pidApp, char *infile, int &ACflg, int spj,
    ...
    while (1) {
        ...
        if (((tick & 0xff) == 0x00) && (!(spj && outFileSize == 0)) &&
            get_file_size(userfile) > outFileSize * 2 + 1024) {
            ACflg = OJ_OL;
            ptrace(PTRACE_KILL, pidApp, NULL, NULL);
            break;
        }
...
int get_sim(int solution_id, int lang, int pid, int &sim_s_id) {
    ...
    if (!sim) {
        ...
        //c cpp will
        if (lang == 0)
            execute_cmd("/bin/ln ../data/%d/ac/%d.%s ../data/%d/ac/%d.%s 2>/dev/null",
                        pid, solution_id, lang_ext[lang], pid, solution_id, lang_ext[lang + 1]);
        if (lang == 1)
            execute_cmd("/bin/ln ../data/%d/ac/%d.%s ../data/%d/ac/%d.%s 2>/dev/null",
                        pid, solution_id, lang_ext[lang], pid, solution_id, lang_ext[lang - 1]);
    } else {
...
int main(int argc, char **argv) {
    ...
    if (p_id == 0) {
        ...
        if (pidApp == 0) {
            ...
            watch_solution(pidApp, infile, ACflg, spj, userfile, outfile, solution_id, lang, topmemory,
                           mem_lmt, usedtime, time_lmt, p_id, PEflg, work_dir);
        }
    ...
    if (spj != 2) {
        if (oi_mode) {
            ...
            update_solution(solution_id, ACflg, usedtime, topmemory >> 10, sim, sim_s_id, pass_rate);
        }
    } else {
        char src_pth[BUFFER_SIZE];
